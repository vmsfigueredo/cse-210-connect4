        ...
        if(self._keyboard_service.is_key_down("K")):
            if(self._execute == True):
                ...
                if ghost.get_next_position().equalsToAny(p1_pieces) == False and ghost.get_next_position().equalsToAny(p2_pieces) == False:
                    if(self._turns % 2 == 0):
                        cast.add_actor("p1_pieces", Piece(ghost.get_position(), constants.PLAYER1))
                    else:
                        cast.add_actor("p2_pieces", Piece(ghost.get_position(), constants.PLAYER2))
                    self._turns += 1
        ...
        for piece in p1_pieces:
            if piece.get_falling() == False:
                ...
                if( piece.get_next_position(Point(constants.CELL_SIZE, 0)).equalsToAny(p1_pieces) and
                    ...
                    piece.get_next_position(Point(constants.CELL_SIZE * 3, 0)).equalsToAny(p1_pieces)
                ): self._winner = "p1"
                # Vertical
                if( piece.get_next_position(Point(0, constants.CELL_SIZE)).equalsToAny(p1_pieces) and
                    ...
                    piece.get_next_position(Point(0, constants.CELL_SIZE * 3)).equalsToAny(p1_pieces)
                ): self._winner = "p1"
                # Diagonal
        ...
        for piece in p2_pieces:
            if piece.get_falling() == False:
                ...
                if( piece.get_next_position(Point(constants.CELL_SIZE, 0)).equalsToAny(p2_pieces) and
                    ...
                    piece.get_next_position(Point(constants.CELL_SIZE * 3, 0)).equalsToAny(p2_pieces)
                ): self._winner = "p2"
                # Vertical
                if( piece.get_next_position(Point(0, constants.CELL_SIZE)).equalsToAny(p2_pieces) and
                    ...
                    piece.get_next_position(Point(0, constants.CELL_SIZE * 3)).equalsToAny(p2_pieces)
                ): self._winner = "p2"
                # Diagonal
